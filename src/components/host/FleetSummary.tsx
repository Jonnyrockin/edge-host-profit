      ...
      <CardContent>
        ...
        <div className="mt-4 space-y-2">
          <div className="flex justify-between items-center p-3 bg-card-hover rounded">
            <span className="text-foreground/70 text-sm">Monthly Breakdown:</span>
            <span className="font-semibold">${Math.round(metrics.grossRevenue).toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-card-hover rounded">
            <span className="text-foreground/70 text-sm">Gross Revenue</span>
            <span className="font-semibold">${Math.round(metrics.grossRevenue).toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-card-hover rounded">
            <span className="text-foreground/70 text-sm">Costs / Expenses (OPEX)</span>
            <span className="text-destructive font-semibold">-${Math.round(metrics.totalOpex).toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-card-hover rounded">
            <span className="text-foreground/70 text-sm">Platform Fee (25%)</span>
